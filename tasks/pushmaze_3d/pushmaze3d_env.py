...
class PushMaze3DEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: PushMaze3DEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        for joint_name in cfg.actuated_joint_names:
            ...
            print("Joint name and index:", joint_name, self.robot.joint_names.index(joint_name))

        # probe body
        self.probe_body = list()
        for body_name in self.cfg.probe_body_name:
            self.probe_body.append(self.robot.body_names.index(body_name))
        ...
        # create action scale and target joint position tensor
    ...
    def _apply_action(self) -> None:
        ...
        self.joint_pos_target = torch.clamp(
            self.joint_pos + self.action_scale * self.actions,
    ...
    def _get_observations(self) -> dict:
        ...
        obs_policy = torch.cat(
            (
                self.robot.data.joint_pos.view(self.num_envs, -1),  # robot position (6)
                self.robot.data.joint_vel.view(self.num_envs, -1),  # robot velocity (6)
    ...
    def get_rewards_from_obs(self, obs_policy: torch.Tensor) -> torch.Tensor:
        ...
        object_pos_xy = obs_policy[:, 12:14]
        object_rotation = obs_policy[:, 14:18]
        goal_pos_xy = obs_policy[:, 23:25]
        goal_rot = obs_policy[:, 25:29]
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None) -> None:
        ...
        if self.reset_dist_type == "train":
            ...
            dof_pos = torch.tensor(self.cfg.default_hand_joint_pos, device=self.device).repeat((len(env_ids), 1))
            dof_vel = torch.zeros_like(dof_pos, device=self.device)
            # sample random position for object
            obj_pos_xy = torch.tensor(self.cfg.default_object_pos[0:2], device=self.device).repeat(len(env_ids), 1)
            obj_rot = torch.tensor(self.cfg.default_object_quat, device=self.device).repeat(len(env_ids), 1)
    ...
    def _reset_target_pose(self, env_ids: torch.Tensor) -> None:
        ...
        goal_height = self.object.data.default_root_state.clone()[:, 2] - 0.05
        goal_pos = torch.cat(
            [
                self.goal_pos_xy,
    ...
    def _compute_intermediate_values(self) -> None:
        ...
        probe_pos = self.robot.data.body_pos_w[:, self.probe_body]
        self.probe_pos = probe_pos[:, 0, :]
    ...
    def get_env_states(self) -> torch.Tensor:
        ...
        return torch.cat(
            [
                self.joint_pos.detach().clone(),  # (6)
                self.joint_vel.detach().clone(),  # (6)
    ...
    def set_env_states(self, states, env_ids: torch.Tensor) -> None:
        ...
        joint_pos = states[:, :6]
        joint_vel = states[:, 6:12]
        object_xy_pos = states[:, 12:14]
        object_rot = states[:, 14:18]
        object_lin_vel_xy = states[:, 18:20]
        object_ang_vel = states[:, 20:23]
    ...
    def q_to_goal(self, q: torch.Tensor) -> torch.Tensor:
        ...
        goal = q[:, 12:18]  # extract obj_pos_xy and obj_rot from q_state
        return goal
    ...
    def compute_goal_distance(self, prm_nodes: torch.Tensor, goal: torch.Tensor) -> torch.Tensor:
        ...
        obj_pos_dist = torch.linalg.norm(prm_nodes[:, 12:14] - goal[12:14], dim=1)
        # Compute object rotation
        obj_rot_dist = compute_quat_angle(goal[14:18], prm_nodes[:, 14:18]).squeeze()
    ...
    def compute_distance(self, selected_node: torch.Tensor, prm_nodes: torch.Tensor) -> torch.Tensor:
        ...
        robot_pos_dist = 0.5 * torch.linalg.norm(prm_nodes[:, 0:6] - selected_node[0:6], dim=1)
        # Robot velocity distance
        robot_vel_dist = 0.005 * torch.linalg.norm(prm_nodes[:, 6:12] - selected_node[6:12], dim=1)
        # Object position distance
        object_pos_xy_dist = 1.0 * torch.linalg.norm(prm_nodes[:, 12:14] - selected_node[12:14], dim=1)
        # Object rotation distance
        object_rot_dist = 1.0 * compute_quat_angle(selected_node[14:18], prm_nodes[:, 14:18]).squeeze()
        # Object linear velocity distance
        object_lin_vel_xy_dist = 0.005 * torch.linalg.norm(prm_nodes[:, 18:20] - selected_node[18:20], dim=1)
        # Object angular velocity distance
        object_ang_vel_dist = 0.005 * torch.linalg.norm(prm_nodes[:, 20:23] - selected_node[20:23], dim=1)
    ...
    def sample_q(self, num_samples: int = 32) -> torch.Tensor:
        ...
        # Uniformly sample random robot joint positions
        robot_pos = torch_rand_float(0.0, 1.0, (num_samples, 6), device=self.device)
    ...
    def is_invalid(self, debug: bool = False) -> (torch.Tensor, torch.Tensor):
        ...
        invalid = torch.where(
            self.object_height > self.cfg.max_height,
            torch.ones_like(self.reset_buf),
            invalid,
        )
        invalid = torch.where(
            self.object_height < self.cfg.min_height,
            torch.ones_like(self.reset_buf),
            invalid,
        )
        ...
        probe_obj_dist = torch.linalg.norm(self.object_pos_xy - self.probe_pos, dim=1)
        invalid = torch.where(
            probe_obj_dist > self.cfg.valid_probe_object_thres,
            torch.ones_like(self.reset_buf),
            invalid,
        )
