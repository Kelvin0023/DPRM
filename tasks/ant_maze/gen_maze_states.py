import numpy as np
import os

import matplotlib.pyplot as plt


class Box:
    def __init__(self, l, b, x, y):
        self.l = l
        self.b = b
        self.x = x
        self.y = y

    def isinside(self, xp, yp, pad=1.0):
        xbound = (xp > self.x - 0.5 * self.l - pad) and (
            xp < self.x + 0.5 * self.l + pad
        )
        ybound = (yp > self.y - 0.5 * self.b - pad) and (
            yp < self.y + 0.5 * self.b + pad
        )
        inside = xbound and ybound
        return inside


class Maze:
    def __init__(self, xrange=(-9.0, 9.0), yrange=(-9.0, 9.0)):
        self.obstacles = []
        self.xrange = xrange
        self.yrange = yrange

        self.areas = np.array([obs.area for obs in self.obstacles])

    def sample(self):
        while True:
            x = np.random.uniform(*self.xrange)
            y = np.random.uniform(*self.yrange)

            inside = False
            for obs in self.obstacles:
                if obs.isinside(x, y):
                    inside = True
                    break
            if not inside:
                return x, y

class MazeA(Maze):
    def __init__(self, xrange=(-12.0, 12.0), yrange=(-12.0, 12.0)):
        super().__init__(xrange=xrange, yrange=yrange)

        self.obstacles = [
            Box(7.0, 1.0, 6.0, 3.0),
            Box(1.0, 7.0, 3.0, 6.0),
            Box(7.0, 1.0, -6.0, -3.0),
            Box(1.0, 7.0, -3.0, -6.0),
            Box(7.0, 1.0, 6.0, -3.0),
            Box(1.0, 7.0, 3.0, -6.0),
            Box(7.0, 1.0, -6.0, 3.0),
            Box(1.0, 7.0, -3.0, 6.0),
        ]

class MazeB(Maze):
    def __init__(self, xrange=(-9.0, 9.0), yrange=(-9.0, 9.0)):
        super().__init__(xrange=xrange, yrange=yrange)

        self.obstacles = [
            Box(0.5, 8.0, -4.0, 0.0),
            Box(2.5, 0.5, -2.5, 4.0),
            Box(2.5, 0.5, -2.5, -4.0),
            Box(0.5, 8.0, 4.0, 0.0),
            Box(2.5, 0.5, 2.5, 4.0),
            Box(2.5, 0.5, 2.5, -4.0),
        ]


class MazeC(MazeB):
    def __init__(self, xrange=(-12.0, 12.0), yrange=(-12.0, 12.0)):
        super().__init__(xrange=xrange, yrange=yrange)

        self.obstacles.extend(
            [
                Box(14.0, 0.5, 0.0, 7.0),
                Box(0.5, 5.0, -7.0, 4.5),
                Box(0.5, 5.0, 7.0, 4.5),
                Box(14.0, 0.5, 0.0, -7.0),
                Box(0.5, 5.0, -7.0, -4.5),
                Box(0.5, 5.0, 7.0, -4.5),
            ]
        )


def generate(maze, nsample=100):
    states = np.zeros((nsample, 4))

    for i in range(nsample):
        x, y = maze.sample()
        states[i, 0] = x
        states[i, 1] = y

    return states


def plot(states, pngfile):
    plt.plot(states[:, 0], states[:, 1], "b.")
    plt.savefig(pngfile)


if __name__ == "__main__":
    maze = MazeA()
    name = "maze_a"

    npfile = "tasks/{}.npy".format(name)
    pngfile = "tasks/{}.png".format(name)
    os.makedirs(os.path.dirname(npfile), exist_ok=True)
    state = generate(maze, nsample=10000)
    state = state.astype(dtype=np.float32)

    with open(npfile, "wb") as f:
        np.save(f, state)

    plot(state, pngfile)
