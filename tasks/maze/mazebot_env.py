...
from tasks.maze.mazebot_env_cfg import MazeBotEnvCfg
from tasks.maze.gen_maze_states import MazeA, MazeB, MazeC, generate
from utils.misc import AverageScalarMeter, to_torch


class MazeBotEnv(DirectRLEnv):
    cfg: MazeBotEnvCfg
    def __init__(self, cfg: MazeBotEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # set up reset distribution (only used to test the implementation of tasks)
        self.reset_state_buf = to_torch(generate(self.maze_object, 5000), device=self.device)

        # planning state (x) dimension
        self.planning_state_dim = 4  # 2 for robot position and 2 for robot velocity
    ...
    def _get_observations(self) -> dict:
        ...
        obs_policy = torch.cat(
            (
                self.robot.data.joint_pos.view(self.num_envs, -1),
                self.robot.data.joint_vel.view(self.num_envs, -1),
                self.goal
            ),
            dim=-1,
        )
    ...
    def compute_reward_in_walks(self, expand_state) -> torch.Tensor:
        ...
        expand_state[:, 0:2]: joint position
        expand_state[:, 2:4]: joint velocity
        expand_state[:, 4:6]: goal position
        """
    ...
    def get_env_states(self) -> torch.Tensor:
        ...
        return torch.cat([self.joint_pos.detach().clone(), self.joint_vel.detach().clone()], dim=1)
    ...
    def compute_goal_distance(self, prm_nodes: torch.Tensor, goal: torch.Tensor) -> torch.Tensor:
        ...
        """
        # In the MazeBot task, the goal is the position of the node
        nodes_pos = prm_nodes[:, 0:2].view(-1, 2)  # extract position from the node
    ...
    def compute_distance(self, selected_node: torch.Tensor, prm_nodes: torch.Tensor) -> torch.Tensor:
        ...
        # Position distance
        pos_dist = 1.0 * torch.linalg.norm(prm_nodes[:, :2] - selected_node[:2], dim=1)
        # Velocity distance
        vel_dist = 0.05 * torch.linalg.norm(prm_nodes[:, 2:4] - selected_node[2:4], dim=1)
    ...
    def sample_random_nodes(self, N: int = 32) -> torch.Tensor:
        ...
        init_states = generate(self.maze_object, N)
        q = to_torch(init_states, device=self.device)[:, :2]
        # Sample random velocity within velocity limits
        ...
        q_dot = alpha * self.dof_vel_upper_lim + (1 - alpha) * self.dof_vel_lower_lim
        # Concatenate position and velocity
        x_start = torch.cat([q, q_dot], dim=1)
        return x_start
    ...
    def is_invalid(self, debug: bool = False) -> (torch.Tensor, torch.Tensor):
        ...
        # No position constraints in the MazeBot task as the collison is handled by the physics engine
