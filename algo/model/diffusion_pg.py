...
from algo.model.diffusion import DiffusionModel, Sample
from algo.diffusion.auxiliary import extract_into_tensor, make_timesteps


class DiffusionPG(DiffusionModel):
    def __init__(
        self,
        actor_model,
        critic_model,
    ...
    def p_mean_var(
        ...
        """
        Get mean and variance of the policy distribution
        """
        ...
        if self.predict_epsilon:
            if self.use_ddim:
                ...
                sqrt_one_minus_alpha = extract_into_tensor(
                    self.ddim_sqrt_one_minus_alpha_t, index, x.shape
                )
    ...
    def get_logprobs(self, cond, chains, get_entropy: bool=False):
        """
        ...
        Args:
            cond: dict with key state/rgb; more recent obs at the end
                ...
                rgb: (B, To, C, H, W)
            chains: (B, K+1, Ta, Da)
            get_entropy: flag for returning entropy
            use_base_policy: flag for using base policy

        Returns:
            logprobs: (B x K, Ta, Da)
            entropy (if get_entropy=True):  (B x K, Ta)
        """
        ...
        chains_prev = chains_prev.reshape(-1, self.action_horizon, self.action_dim)
        chains_next = chains_next.reshape(-1, self.action_horizon, self.action_dim)
    ...
    def loss(self, cond, chains, reward):
        ...
        logprobs, eta = self.get_logprobs(cond, chains, get_entropy=True)
        ...
        # -> (n_steps x n_envs) x K x Ta
        logprobs = logprobs.reshape((-1, self.denoising_steps, self.action_horizon))
